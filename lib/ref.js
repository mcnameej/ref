 ...
 * C String is read until the first NUL byte, which indicates the end of the
 * String.
 *
 ...
 * @param {Number} offset The offset to begin reading.
...
/**
 * Returns a JavaScript String read from _buffer_ at the given _offset_.
 * The Wide C String is read until the first NUL character, which indicates
 * the end of the String.
 *
 * Similar to `readCString()`, but with __ucs2__ encoding (Windows UNICODE).
 *
 * This function can read beyond the `length` of a Buffer.
 *
 * _offset_ is expressed in __bytes__, not characters.
 * Each ucs2 wide character takes two bytes.
 *
 * ```
 * var buf = new Buffer('hello\0world\0', 'ucs2');
 *
 * var str = ref.readWString(buf, 0);
 * console.log(str);
 * 'hello'
 * ```
 *
 ...
 *   * `"string"` gets coerced into `ref.types.CString`.
 *   * `"wstring"` gets coerced into `ref.types.WString`.
 *
...
exports.coerceType = function coerceType (type) {
  ...
  if (typeof rtn === 'string') {
    ...
    if (rtn === 'pointer') {
      ...
      rtn = exports.types.WString // special wchar_t * type
    } else {
      var refCount = 0
 ...
 * @return {Buffer} The new `Buffer` instance with _string_ written to it, and a trailing NUL byte.
 */
...
/**
 * Returns a new `Buffer` instance with the given String written to it using
 * ucs2 encoding. The buffer is 2 bytes longer than the string itself, and
 * is NUL terminated.
 *
 * Similar to `allocCString()`, but with __ucs2__ encoding (Windows UNICODE).
 *
 * ```
 ...
 * ```
 *
 * @param {String} string The JavaScript string to be converted to a wide C string.
 * @return {Buffer} The new `Buffer` instance with _string_ written to it, and a NUL terminator.
 */

exports.allocWString = function allocWString (string) {
  if (null == string || (Buffer.isBuffer(string) && exports.isNull(string))) {
    return exports.NULL
  }
  var size = Buffer.byteLength(string, 'ucs2') + 2
  var buffer = new Buffer(size)
  exports.writeWString(buffer, 0, string)
  buffer.type = wcharPtrType
  return buffer
}

/**
 * Writes the given string as a C String (NUL terminated) to the given buffer
 ...
 *
 * @param {Buffer} buffer The Buffer instance to write to.
 * @param {Number} offset The offset within the buffer to begin writing.
 * @param {String} string The JavaScript String that will be written to the buffer.
...
exports.writeCString = function writeCString (buffer, offset, string, encoding) {
  ...
  // BUG: A single zero byte is an insufficient terminator with ucs2 encoding,
  // which is technically an option with writeCString(). I suspect people are
  // only using utf8 and/or ascii. allocCString() is similarly broken with ucs2.
  // Not sure if CString functions should be fixed; leaving them alone for now.
  // The WString variants do the right thing.
}

/**
 * Writes the given string as a Wide C String (NUL terminated) to the given buffer
 * at the given offset.
 *
 * Similar to `writeCString()`, but with __ucs2__ encoding (Windows UNICODE).
 *
 * Unlike `readWString()`, this function requires the buffer to actually have the
 * proper length.
 *
 * _offset_ is expressed in __bytes__, not characters.
 * Each ucs2 wide character takes two bytes.
...
exports.writeWString = function writeWString (buffer, offset, string) {
  assert(Buffer.isBuffer(buffer), 'expected a Buffer as the first argument')
  assert.equal('string', typeof string, 'expected a "string" as the third argument')
  if (!offset) {
    offset = 0
  ...
  var size = buffer.length - offset
  buffer.write(string.concat(String.fromCharCode(0)), offset, size, 'ucs2')
}
...
if (process.platform === 'win32') {
  /**
   * Returns the Windows HANDLE read from _buffer_ at the given _offset_.
   *
   * If the returned value will fit inside a JavaScript Number without losing
   * precision, then a Number is returned, otherwise a String is returned.
   *
   * @param {Buffer} buffer The buffer instance to read from.
   * @param {Number} offset The offset within the buffer to begin reading.
   * @return {HANDLE} The Windows HANDLE that was read from _buffer_.
   */

  exports.readHANDLE = function readHANDLE (buffer, offset) {
    if (exports.sizeof.pointer === 4)
      return buffer.readUInt32LE(offset || 0)
    else
      return buffer.readUInt64LE(offset || 0)
  }

  /**
   * Writes the Windows HANDLE _value_ into _buffer_ at the given _offset_.
   *
   * @param {Buffer} buffer The buffer instance to write to.
   * @param {Number} offset The offset within the buffer to begin writing.
   * @param {HANDLE} value The Windows HANDLE to be written.
   */

  exports.writeHANDLE = function writeHANDLE (buffer, offset, value) {
    if (exports.sizeof.pointer === 4)
      return buffer.writeUInt32LE(value, offset || 0)
    else
      return buffer.writeUInt64LE(value, offset || 0)
  }

  /**
   * The `HANDLE` value returned by some Windows API's (e.g. CreateFile)
   * when they fail.
   *
   * @type {HANDLE}
   */

  exports.INVALID_HANDLE_VALUE = (exports.sizeof.pointer === 4) ? 0xFFFFFFFF : '18446744073709551615'

  // INVALID_HANDLE_VALUE is defined in C as ``(HANDLE) -1``, which is hard
  // to represent in JavaScript when HANDLE is a uint64.
  //
  // '18446744073709551615' is the equal to 0xFFFFFFFFFFFFFFFF.

  /**
   * Returns the Windows BOOL read from _buffer_ at the given _offset_.
   *
   * @param {Buffer} buffer The buffer instance to read from.
   * @param {Number} offset The offset within the buffer to begin reading.
   * @return {Boolean} true or false.
  ...
  exports.readBOOL = function readBOOL (buffer, offset) {
    return buffer.readInt32LE(offset || 0) ? true : false
  }

  /**
   * Writes the Windows BOOL _value_ into _buffer_ at the given _offset_.
   *
   * @param {Buffer} buffer The buffer instance to write to.
   * @param {Number} offset The offset within the buffer to begin writing.
  ...
  exports.writeBOOL = function writeBOOL (buffer, offset, value) {
    if (typeof value !== 'number') {
      value = value ? 1 : 0
    }
    return buffer.writeInt32LE(value, offset || 0)
  }
}
 ...
 *
 * @param {Buffer} buffer A Buffer instance to base the returned Buffer off of.
 * @param {Number} size The number of sequential, aligned `NUL` bytes that are required to terminate the buffer.
 * @param {Number} offset The offset of the Buffer to begin from.
...
/**
 * Accepts a `Buffer` instance and a number of `NUL` bytes to read from the
 * pointer. This function will scan past the boundary of the Buffer's `length`
 * until it finds `size` number of aligned `NUL` bytes.
 *
 ...
 *
 * @param {Buffer} buffer A Buffer instance to base the returned Buffer off of.
 * @param {Number} size The number of sequential, aligned `NUL` bytes are required to terminate the buffer.
 * @param {Number} offset The offset of the Buffer to begin from.
 * @return {Buffer} A new Buffer instance with the same memory address as _buffer_, and a variable `length` that is terminated by _size_ NUL bytes.
 */

exports.reinterpretUntilZeros = function reinterpretUntilZeros (buffer, size, offset) {
  debug('reinterpreting buffer to until "%d" NUL (0) bytes are found', size)
 ...
 * The `WString` type.
 *
 * WStrings are a kind of weird thing. We say it's `sizeof(wchar_t *)`, and
 * `indirection` level of 1, which means that we have to return a Buffer that
 * is pointer sized, and points to a some ucs2 string data, so we have to create
 * a 2nd "in-between" buffer.
 *
 * And now, a few notes for people who don't work on Windows regularly...
 *
 * TL;DR -- To get correct handling of non-ASCII characters, you must use
 * wide strings, and the UNICODE version of the API, when interfacing with
 * Windows. node's built-in modules (e.g. File System) already do this
 * (via libuv), but you need to do it yourself when calling Windows API's
 * directly (e.g. via the foreign function interface).
 *
 * wchar_t is defined by the C/C++ standard and available on most POSIX
 * platforms, but is most often used on Windows. UCS-2 encoded wide strings
 * are the native string format of the Windows kernel and Win32 API.
 *
 * Most Win32 API's are available in "ANSI" and "UNICODE" versions (with
 * "A" and "W" suffixes on the function name). Structures with embedded
 * strings (or string pointers) will also have ANSI and UNICODE versions.
 * The Windows header files use a lot of #define hackery to hide the mess.
 *
 * In almost all cases, the ANSI version of an API simply converts strings
 * to UCS-2 and calls the UNICODE version to do the actual work. Some API's
 * are only provided in UNICODE, and some features may only be available
 * via the UNICODE version.
 *
 * It is important to understand that "ANSI" doesn't mean UTF-8 encoding.
 * ANSI strings use a code page (https://en.wikipedia.org/wiki/Code_page).
 * Some code pages support double byte characters (used in Asia), but most
 * only define 256 characters. None can represent the full range of Unicode
 * characters, so Unicode to ANSI conversion may not be possible. ANSI to
 * to Unicode conversion requires knowing what code page was used to create
 * the string; the code page isn't included in the string data.
 *
 * Programmers who grew up in the Unicode era don't appreciate how bad
 * things used to be. Today you just encode everyting with UTF-8, don't
 * worry, and be happy. We had to encode our characters up hill, both ways,
 * in the snow. And we liked it! Now get off my lawn! :-)
 *
 * Microsoft designed Win32 in the late 80's and early 90's. They knew
 * code pages were a kludge, and needed something better, so they adopted
 * the Unicode standard and UCS-2 encoding (16-bit wide characters).
 * Let's give Microsoft credit for being ahead of most commercial Unix's
 * of that era in i18n support.
 *
 * Windows provided ANSI API's to ease porting of old code, but Windows
 * exclusively used Unicode internally. Microsoft may have expected the
 * ANSI API's to fade away as the rest of the industry caught up and
 * adopted Unicode. What they didn't foresee was UTF-8. The industry
 * did adopt Unicode, but not UCS-2.
 *
 * Microsoft had the bad luck of adopting Unicode too early. UCS-2 was
 * the one and only encoding at the time, so it's natural that Win32
 * is based on wide characters. The POSIX world didn't really get on
 * board with Unicode until after UTF-8, so POSIX got to keep using
 * _char*_ in all their API's.
 *
 * The `CString` type works great on POSIX, freely converting Unicode
 * strings between UTF-8 and whatever encoding the JavaScript engine
 * uses internally. The conversions are lossless in both directions.
 * CString also works with the ANSI version of Windows API's, as long
 * as you're dealing with 7-bit ASCII characters, but fails with
 * characters U+0080 and above. `WString` is needed for these cases.
 */
...
types.WString = {
  ...
  , get: function get (buf, offset) {
      ...
      return _buf.readWString(0)
    }
  , set: function set (buf, offset, val) {
      var _buf = exports.allocWString(val)
      return buf.writePointer(_buf, offset)
    }
}
...
if (process.platform === 'win32') {
  /**
   * __The `HANDLE` type is only defined on Windows.__
   *
   * Windows handles are modeled as unsigned integers the size of a pointer.
   * In almost all cases, code treats handles as opaque values, returned
   * by one function and passed to others.  In a very few cases, code may
   * need to inspect the numeric value of a handle (e.g. return from
   * ShellExecute). Defining handles as integers doesn't harm the common
   * case (opaque value), but makes it easier to handle the uncommon ones.
   *
   * The JavaScript Number type uses floating point, so it isn't possible
   * to represent large uint64 values as a Number without data loss.
   * Handle values that won't fit in a Number are stored as a String.
   */

  types.HANDLE = {
      size: exports.sizeof.HANDLE
    , alignment: exports.alignof.HANDLE
    , indirection: 1
    , get: function get (buf, offset) {
        return buf.readHANDLE(offset || 0)
      }
    , set: function set (buf, offset, val) {
        return buf.writeHANDLE(val, offset || 0)
      }
  }

  /**
   * __The `BOOL` type is only defined on Windows.__
   *
   * Wrapper type around `types.int32` that accepts/returns `true` or
   * `false` Boolean JavaScript values.
   *
   * "BOOL" (upper case) on Windows is defined (via typedef) as a
   * 32-bit integer, while "bool" (lower case) is a C/C++ language type
   * (size not defined by C/C++ standard, but implemented as an 8-bit
   * integer by the Microsoft C/C++ compiler). Windows API's use BOOL.
   *
   * Note: The Windows kernel uses BOOLEAN, which is a typedef for an
   * unsigned char. node doesn't run in the kernel (at least not yet!),
   * so we won't worry about BOOLEAN.
   */

  types.BOOL = {
      size: exports.sizeof.int32
    , alignment: exports.alignof.int32
    , indirection: 1
    , get: function get (buf, offset) {
        return buf.readBOOL(offset || 0)
      }
    , set: function set (buf, offset, val) {
        return buf.writeBOOL(val, offset || 0)
      }
  }
}
...
;[ 'bool', 'byte', 'char', 'uchar', 'wchar_t', 'short', 'ushort', 'int', 'uint'
, 'long', 'ulong', 'longlong', 'ulonglong', 'size_t' ].forEach(function (name) {
...
/*!
 * This `wchar_t *` type is used by "allocWString()" above.
 */

var wcharPtrType = exports.refType(exports.types.wchar_t)
...
Buffer.prototype.readWString = function readWString (offset) {
  return exports.readWString(this, offset)
}
...
Buffer.prototype.writeWString = function writeWString (string, offset, encoding) {
  return exports.writeWString(this, offset, string, encoding)
}
...
if (process.platform === 'win32') {
  ...
  Buffer.prototype.readHANDLE = function readHANDLE (offset) {
    return exports.readHANDLE(this, offset)
  }
  ...
  Buffer.prototype.writeHANDLE = function writeHANDLE (val, offset) {
    return exports.writeHANDLE(this, offset, val)
  }
  ...
  Buffer.prototype.readBOOL = function readBOOL (offset) {
    return exports.readBOOL(this, offset)
  }
  ...
  Buffer.prototype.writeBOOL = function writeBOOL (val, offset) {
    return exports.writeBOOL(this, offset, val)
  }
}
...
if (!(exports.NULL instanceof Buffer)) {
  ...
  SlowBuffer.prototype.readWString = Buffer.prototype.readWString
  SlowBuffer.prototype.writeCString = Buffer.prototype.writeCString
  SlowBuffer.prototype.writeWString = Buffer.prototype.writeWString
  ...
  if (process.platform === 'win32') {
    SlowBuffer.prototype.readHANDLE = Buffer.prototype.readHANDLE
    SlowBuffer.prototype.writeHANDLE = Buffer.prototype.writeHANDLE
    SlowBuffer.prototype.readBOOL = Buffer.prototype.readBOOL
    SlowBuffer.prototype.writeBOOL = Buffer.prototype.writeBOOL
  }
}
